...
@WebServlet(name="Grades", urlPatterns={"/Grades.json"})
public class Grades extends HttpServlet{
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		Gson gson = new GsonBuilder().create();
		try{
			List<GradesHelper> objGrades = GeneralService.GetGrades();
			String objGradesJson = gson.toJson(objGrades);
			response.setContentType("application/json");
			response.setCharacterEncoding("UTF-8");
			response.getWriter().write(objGradesJson);
		}catch (Exception ex){
